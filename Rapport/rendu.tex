...
On voit sur les figures \ref{fig.exo2_histogramme2D} et \ref{fig.exo2_histogramme2D_vue_dessus} que le modèle gaussien ne possède qu'un seul mode. Le modèle semble proche des observations, mais pour avoir une mesure de l'écart entre l'histogramme expérimental et le modèle gaussien multi-varié nous allons regarder l'erreur quadratique moyenne.
...
$$
Nous allons considérer que $P(\omega_1) = P(\omega_2) = \frac{1}{2}$ 
\\
L'expression précédente devient alors: $P(\omega_1|x)=\frac{1}{2}*\frac{p(x|\omega_1)}{p(x)}$
\\
Afin d'obtenir $P(x|\omega_2)$, on prend les bords des images de la banque d'échantillon. Pour cela, la fonction \emph{SelectPixelsNonCentre}. 
\\

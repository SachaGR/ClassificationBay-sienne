...
On voit sur les figures \ref{fig.exo2_histogramme2D} et \ref{fig.exo2_histogramme2D_vue_dessus} que le modèle gaussien ne possède qu'un seul mode. Le modèle semble toutefois proche des observations avec un maximum correspondant au maximum de l'histogramme des images d'apprentissage. En utilisant l'histogramme expérimental normalisé et en calculant l'erreur quadratique moyenne $\text{e}_{\text{quad}}$ entre celui-ci et le modèle probabiliste de gaussienne multivariée on trouve :
$$
\text{e}_{\text{quad}} = 3,34.10^{-8}
$$
Ce qu'on considère comme bon avec assez peu d'images d'apprentissage pour construire le modèle.
...
\begin{figure}[H]
\center
\includegraphics[width=12cm]{exo2_skin1.png}
